...
#include <esp_wifi.h>  // Добавлено для управления мощностью Wi-Fi
...
bool initWiFi() {
  ...
  // --------------------------------------------------------------------------
  // ШАГ 2.5: Установка максимальной мощности Wi-Fi
  // --------------------------------------------------------------------------
  // Мощность задаётся в единицах 0.25 дБм
  // 80 = 20 дБм (максимальная мощность для ESP32)
  // Диапазон: от 8 (2 дБм) до 84 (20 дБм)
  // 
  // ВНИМАНИЕ: Вызов ДО WiFi.begin() и ПОСЛЕ WiFi.mode()
  // --------------------------------------------------------------------------
  Serial.print(F("📶 Установка мощности TX: 80 (20 дБм) ... "));
  esp_err_t powerResult = esp_wifi_set_max_tx_power(80);
  if (powerResult == ESP_OK) {
    Serial.println(F("OK"));
  } else {
    Serial.print(F("ОШИБКА (код: "));
    Serial.print(powerResult);
    Serial.println(F(")"));
  }
...
// ============================================================================
// ПОЛУЧЕНИЕ ТЕКУЩЕЙ МОЩНОСТИ ПЕРЕДАТЧИКА
// ============================================================================
// Возвращает мощность в дБм
// ----------------------------------------------------------------------------
int8_t getWiFiTxPower() {
  int8_t power = 0;
  if (esp_wifi_get_max_tx_power(&power) == ESP_OK) {
    // Значение power хранится в единицах 0.25 дБм
    // Преобразуем в дБм: делим на 4
    return power / 4;
  }
  return -1;  // Ошибка получения
}
