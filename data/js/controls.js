...
function setupControls() {
    ...
    // --- КНОПКА СБРОСА ТАЙМЕРА (НОВАЯ) ---
    const btnResetTimer = document.getElementById('btnResetTimer');
    if (btnResetTimer) {
        btnResetTimer.addEventListener('click', () => {
            // МОЖНО МЕНЯТЬ: текст подтверждения
            if (confirm('Сбросить счётчик времени нагрева?')) {
                sendCommand('resetTimer', 0);
            }
        });
    }
...
// 3. ТЕКСТ ПОДТВЕРЖДЕНИЯ: в confirm('...')
//
